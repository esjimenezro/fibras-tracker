...
* **sources/** — transcripción fiel y completa del PDF a markdown (`sources/YYYY-QN-source.md`), incluyendo tablas. Generada **mecánicamente vía la librería** `pymupdf4llm` (`pymupdf4llm.to_markdown()`), **no por el LLM** — esto es lo que garantiza que sea determinística (sin síntesis ni interpretación) y completa (no omite renglones de tablas por criterio editorial, a diferencia de una transcripción hecha por el LLM). Es barata de regenerar si se pierde y no requiere el mismo escrutinio que `pages/`. El OCR automático que usa la librería en páginas con texto embebido en imágenes (portadas, gráficas) puede dejar ruido cosmético — separadores de miles perdidos ocasionalmente, texto decorativo corrupto en páginas de portada/separadores — esto es aceptable y no se corrige a mano ni se reescribe con el LLM: sería reintroducir el riesgo de síntesis que este paso busca evitar. **Autoridad numérica:** cualquier cifra en `sources/` es solo para verificación cruzada — `fundamentals.json` es la única fuente de verdad para cálculos o citas numéricas (pasó por validación de identidad contable, conversión de unidades y correcciones que `sources/` no tiene). Si un número difiere entre ambos, `fundamentals.json` gana.
...
1. Genera la transcripción fiel completa en `sources/YYYY-QN-source.md` corriendo `pymupdf4llm.to_markdown("raw/YYYY-QN.pdf")` (instalar con `uv pip install pymupdf4llm` si falta) y guardando el resultado tal cual — **no leer el PDF ni transcribirlo con el LLM, y no reescribir ni "limpiar" el output de la librería** (ver nota de ruido cosmético en la sección de capas arriba).
